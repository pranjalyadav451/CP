...
int main() {
	...
	while (winner == 0) {
		...
		for (auto a : cnt) (a.second % 2 == 1) ? odd++ : even++;
		...
		if (winner == 0) {
			...
			if (!done) temp.pop_back();
			str = temp;
		}
		turn  = !turn;
	}
	cout << (winner == 1 ? F : S) << endl;
}