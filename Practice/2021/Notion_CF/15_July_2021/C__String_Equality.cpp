...
void solve() {
	...
	for (auto b = cntB.begin(); b != cntB.end(); b++) {
		if (b->second % K != 0) {
			bool found = false;
			int toRemove = b->second % K;
			for (auto a = cntA.begin(); a != cntA.end(); a++) {
				if (a->first == b->first and a->second % K == toRemove) {
					a->second -= toRemove; b->second -= toRemove;
					found = true;
				}
			}
			if (found == false) {
	...
	for (auto b = cntB.begin(); b != cntB.end(); b++) {
		for (auto a = cntA.rbegin(); a != cntA.rend(); a++) {
			if (b->first >= a->first and a->second % K == 0) {
				if (b->second > 0) {
					int need = b->second , have = a->second;
					b->second -= min(need, have);
					a->second -= min(need, have);
				}
			}
		}
		if (b->second != 0) {
