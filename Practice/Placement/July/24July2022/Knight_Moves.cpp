...
const vector<pii> moves = {{1, -2}, {1 , 2}, {2 , 1}, {2, -1}, { -1, -2}, { -1 , 2}, { -2 , 1}, { -2, -1}, };

bool is_valid(int i, int j, int board) {
	return i >= 0 and i < board and j >= 0 and j < board;
}


int get_target(vi S, vi &E, int board) {
	vector<vi> visited(board, vi(board));
	queue<pii>  nodes;

	pii current = {--S[0], --S[1]}, target = {--E[0], --E[1]};
	nodes.push(current);
	...
	while (nodes.size()) {
		int N = nodes.size();
		if (visited[target.first][target.second]) return dist;

		while (nodes.size() and N--) {
			pii now = nodes.front();
			int i = now.first , j = now.second;
			nodes.pop();
			for (auto &move : moves) {
				int x = i + move.first , y = j + move.second;
				if (is_valid(x, y, board) and visited[x][y] == 0) {
					nodes.push({x, y});
					visited[x][y] = 1;
				}
			}
...
public:
	...
	int minStepToReachTarget(vector<int>&start, vector<int>&target, int board) {
		int ans = get_target(start, target, board);
		return ans;
	}
};
...
void solve() {
	...
	in_arr(2, start); in_arr(2, end);;


	Solution a;
	int ans = a.minStepToReachTarget(start, end, n);
	out(ans);
}
int main() {
	int tt = 1;
