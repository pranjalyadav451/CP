/*
name: B - Kleene Inversion
group: AtCoder - Japanese Student Championship 2019 Qualification
url: https://atcoder.jp/contests/jsc2019-qual/tasks/jsc2019_qual_b
interactive: false
memoryLimit: 1024
timeLimit: 2000
Started At: 12:03:54 PM
*/
#include <bits/stdc++.h>
using namespace std;
...
void solve() {
	...
	Modular<> ans = 0;
	for (int i = 0; i < n; i++) {
		Modular<> times = k * (k + 1); times /= 2;
		ans += times * left[i] + (times - k) * right[i];
	}
	cout << ans.value << endl;
}
int main() {
