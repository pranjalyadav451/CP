...
vector<int> MoneyPath(int n, int m, vector<vector<int>> arr) {
    vector<vector<pair<int, int>>> graph(n + 1);
    vector<int> paisa(n + 1);
    for (int i = 0; i < m; i++) {
        int from = arr[i][0], to = arr[i][1], amount = arr[i][2];
        paisa[from] += amount;
        graph[from].push_back({ to,amount });
    }

    long long minMoney = 1, maxMoney = 1;
    for (int i = 1; i <= n + 1; i++) {
        if (paisa[minMoney] > paisa[i]) minMoney = i;
        if (paisa[maxMoney] < paisa[i]) maxMoney = i;
    }
