...
ll get(vll &arr, map<ll, vector<ll>> &mp, int start, int end) {
    if (start > end) return start - end == 1;

    if (dp[start] != -1) return dp[start];

    ll part2 = false, part1 = false;
    if (start + arr[start] <= end)
        part1 = get(arr, mp, start + arr[start] + 1, end);

    ll val = start;
    auto can = mp[start];
    for (ll ele : can) {
        if (not part2) part2 |= get(arr, mp, ele + 1, end);
        else return true;
    }
    return dp[start] = part1 or part2;
}
