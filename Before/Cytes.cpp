...
int get(string &candi, string &str, int i, int j, int k) {
    if (i > str.length() || j > candi.length()) return false;
    if (j == candi.size()) return true;
    ...
    if (candi[j] == str[i]) {
        int igc = get(candi, str, i, j + 1, k);
        ans = igc || get(candi, str, i + 1, j + 1, k) || (k > 0 ? get(candi, str, i + 1, j, k - 1) : 0);
    }
    else {
        int igc = get(candi, str, i, j + 1, k);
        ans = igc or (k > 0 ? get(candi, str, i + 1, j, k - 1) : 0);
    }
    return ans;
}
...
void solve() {
    ...
    int cnt = 0;
    for (int i = 0; i < N; i++) {
        int check = get(tickets[i], str, 0, 0, K);
        if (check) cnt++;
    }
    cout << cnt << endl;
}
