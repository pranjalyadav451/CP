...
public:
    ...
    vector <int> dijkstra(int V, vector<vector<int>> adj[], int S) {
        ...
        while (nodes.size()) {
            ...
            // we don't do
            // visited[now.second] = now.first without the visited check as mentioned in the below commented out
            // code, because there can entry for a node which has higher calculated dist than its minimum possible
            // distance as the set will treat {dist_greater,node} and {smallest_dist,node} differently

            for (auto near : adj[now.second]) {
                int current = near[0], weight = near[1];
                if (dist[current] > now.first + weight) {
                    dist[current] = min(dist[current], now.first + weight);
                    nodes.insert({dist[current], current});
                }
            }
        }
        return dist;
