...
vector<vector<char>> matrix(LIM, vector<char>(LIM, '_'));

bool is_valid(int i , int j) {
	return i >= 0 and i < LIM and j >= 0 and j < LIM;
}
ll nc2(ll x) {
	return (x * (x - 1)) / 2;
}
...
void solve() {
	...
	REP(i, 0, N - 1) {
		...
		cin >> x >> y;
		matrix[--x][--y] = 'B';
		// two coordinates in a matrix are in the same diagonal if
