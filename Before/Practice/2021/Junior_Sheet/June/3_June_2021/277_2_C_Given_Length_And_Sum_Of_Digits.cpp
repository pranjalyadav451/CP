...
int main() {
    ...
    for (int i = 1; i <= m; i++) {
        int number = 9;
        while (number >= 0 and sum - number <= 9 * (m - i)) {
            number--;
            if (i == 1 and number == 0 and sum > 0) break;
        }
        number++;
        sum = sum - number;
        small.push_back(number + '0');
    }
    sum = s;
    for (int i = 1; i <= m; i++) {
        int number = 0;
        while (number <= 9 and sum - number >= 0) {
            number++;
        }
        number--;
        sum = sum - number;
        large.push_back(number + '0');
    }
