...
void solve(vector<string> &sudo) {
	vector<string> temp = sudo;
	int col = 0;

	// vector<pii>coords = {{0, 0}, {1, 3}, {2, 6}, {3, 1}, {4, 4}, {5, 7}, {6, 2}, {7, 5}, {8, 8}};

	// for (pii a : coords) {
	// 	if (sudo[a.first][a.second] == '1') sudo[a.first][a.second] = '2';
	// 	else sudo[a.first][a.second] = '1';
	// }

	for (int i = 0; i < 9; ) {
		col = i / 3;
		for (int j = col; j < 9 ; j += 3, i++) {
			if (sudo[i][j] == '1') sudo[i][j] = '2';
			else sudo[i][j] = '1';
		}
	}
