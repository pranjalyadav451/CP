...
// REALLY GOOD QUESTION OF TWO POINTERS.
...
void solve() {
	...
	/* There are three choices for removing the smallest subarray to make the array sorted.
	 * from prefix ,
	 * from suffix ,
	 * in between prefix and suffix.
	*/
	int from_left = 0, from_right = N - 1, moves = inf;
	while (from_left + 1 < N and arr[from_left] <= arr[from_left + 1]) {
		from_left++;
	}
	while (from_right - 1 >= 0 and arr[from_right - 1] <= arr[from_right]) {
		from_right--;
	}
	moves = min(N - from_left - 1, from_right);


	for (int i = 0, j = from_right; i <= from_left; i++) {
		if (arr[i] <= arr[j]) {
			moves = min(((j - i - 1) > 0 ? j - i - 1 : 0), moves);
		}
		else {
			j++;
		}
