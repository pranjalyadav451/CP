...
int groups(string &str, vpl &arr) {
	int N = str.size();
	int cnt = 0;
	for (int i = 0; i < N; i++) {
		if (str[i] == '1') {
			int start = i;
			while (i < N and str[i] == '1') i++;
			i--;
...
void solve() {
	int act , place; cin >> act >> place;
	string mines; cin >> mines;

	vpl G;
	int cnt = groups(mines, G) , cost = 0;
	int N = G.size();
	for (int i = 0; i < N - 1; i++) {
		pll current = G[i], next = G[i + 1];
		int dist = next.ff - current.ss - 1;
		if (place * dist < act) cost += dist * place ;
		else cost += act;
	}
	cost += cnt > 0 ? act : 0 ;
	cout << cost << endl;
