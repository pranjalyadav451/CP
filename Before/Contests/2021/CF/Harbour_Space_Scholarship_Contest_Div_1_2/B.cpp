...
vector<int> search(string &S , string right) {
	vector<int> res;
	int prev = 0;
	while (S.find(right, prev) != string::npos) {
		int found = S.find(right, prev);
		res.push_back(found);
		prev = found + 1;
	}
	return res;
...
void solve() {
	...
	// When we place the chip and our first move is towards left.
	...
	for (int i = 0; i < T.size(); i++) {
		string right = T.substr(0, i + 1), left = (i != N - 1 ? T.substr(i + 1) : "");

		is = false;

		if (S.find(right) != string::npos) {
			vector<int> foundAt = search(S, right);

			for (int found : foundAt) {
				string revS = S.substr(0, found + right.size() - 1);
				reverse(all(revS));

				if (left.size() <= revS.size()) {
					bool ok = true;
